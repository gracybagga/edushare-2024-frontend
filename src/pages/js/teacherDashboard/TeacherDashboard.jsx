            ...
            try {
                if(userRole==='TEACHER') {
                    ...
                    if (!teacherResponse.ok) {
                        throw new Error('Teacher dashboard could not be fetched.');
                    }
                    const teacherResult = await teacherResponse.json(); // parse the response
                    localStorage.setItem('teacherId',teacherResult.teacherId);
                    setEnrolledCourses(teacherResult.coursesAssigned); // set student's coursesEnrolled
                }
            } catch (error) {
